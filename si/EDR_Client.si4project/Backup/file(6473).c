...
#endif
{
	...
	printf("check_illegal_script  match [%d] @%s line:%d\r\n", match, __FILE__,__LINE__);
...
#endif
{
	...
	if (rep->type == F_ILLEGAL_SCRIPT &&
		...
		printf("check_illegal_script unmatch[%s] @%s line:%d\r\n", keyword, __FILE__,__LINE__);
		return;
	}
...
#if 0
int check_to_report(char *path, filereq_t *rep)
#else
int check_to_report(char *path, struct ebpf_filereq_t *rep)
#endif
{
	...
	/* 文件内容没有改变 */
	if (st.st_mtim.tv_sec == rep->mtime_sec &&
	    st.st_mtim.tv_nsec == rep->mtime_nsec) {
		return 0;
	}
...
//	backup_size = check_dir_size(TRASH_DIR);
	...
	while (Online) {
		...
		if (sniper_file_loadoff == TURN_MY_ON) {
			...
			printf("sniper_file_loadoff @%s line:%d\r\n",__FILE__,__LINE__);
			continue;
	  	}
...
#endif
		if (rep == NULL) {
			printf("kfile_msg->data is null @%s line:%d\r\n",__FILE__,__LINE__);
			continue;
		}
		...
		if(strcmp(rep->comm,"vim")==0){
			if(strstr(rep->filename,".php")) rep->type=F_ILLEGAL_SCRIPT;
			if(strstr(rep->filename,".jsp")) rep->type=F_ILLEGAL_SCRIPT;
			if(strstr(rep->filename,".as")) rep->type=F_ILLEGAL_SCRIPT; 
			if(strstr(rep->filename,".cdx")) rep->type=F_ILLEGAL_SCRIPT; 
			if(strstr(rep->filename,".cer")) rep->type=F_ILLEGAL_SCRIPT;   
			if(strstr(rep->filename,".cgi")) rep->type=F_ILLEGAL_SCRIPT;
...
//				MON_ERROR("filename is NULL\n");
		...
		if (rep->type != F_PRINTER && rep->type != F_CDROM) {
			...
			msg.pathname_new[PATH_MAX-1] = 0;
			if ((msg.pathname[0] == '\0') || (rep->path_len == 0)){
				// printf("filename is null @%s line:%d\r\n",__FILE__,__LINE__);
				MON_ERROR("filename is NULL\n");				
				continue;
			}
		}
		...
		if (rep->op_type == OP_RENAME || rep->type == F_ENCRYPT_BACKUP) {
			if ((msg.pathname_new[0] == '\0') || (rep->newpath_len == 0)){
				...
				printf("newfilename is null @%s line:%d\r\n",__FILE__,__LINE__);
				continue;
			}
		}
		...
		if (rep->op_type == OP_OPEN_W && 
		   (rep->type != F_BLACK_AFTER &&
			...
			if (!check_to_report(msg.pathname, rep)) {
				...
				printf("check file msg to report, op_type:%d, type:%d @%s line:%d\r\n",rep->op_type, rep->type,__FILE__,__LINE__);
				continue;
			}
		}
