...
void *kfile_msgd(void *ptr)
{
	...
	if (!file_program_obj) printf("get_bpf_object:NULL@%s line:%d\r\n",__FILE__,__LINE__);

	// 在ebpf对象中查找名为filereq_ringbuf的map
	filereq_ringbuf_map = bpf_object__find_map_by_name(file_program_obj->, "filereq_ringbuf");
	if (!filereq_ringbuf_map) printf("bpf_object__find_map_by_name:NULL@%s line:%d\r\n",__FILE__,__LINE__);
	...
	if (!filereq_ringbuf) printf("filereq_ringbuf is null @%s line:%d\r\n",__FILE__,__LINE__);
	...
	while (Online) {
		...
		if (err < 0) printf("ring_buffer__poll err[%d]@%s line:%d\r\n", err,__FILE__,__LINE__);	
#endif
		print_droped_file_msgs();
	}
