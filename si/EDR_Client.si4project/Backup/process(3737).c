...
#endif
{
	...
	printf("EXEC process: %s(%d), nodename: %s(%u)@%s line:%d\n", req->cmd, req->pid, req->nodename, req->mnt_id,__FILE__,__LINE__);
...
// TODO(luoyinhong)
		...
		if (is_danger_cmd(taskstat->args)) {
			...
			printf("danger taskstat %s: %s\ncwd: %s@%s line:%d\n", taskstat->cmd, taskstat->args, taskstat->cwd,__FILE__,__LINE__);
		}
