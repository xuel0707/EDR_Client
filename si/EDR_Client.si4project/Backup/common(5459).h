...
struct net_request {
	...
	pid_t pid;                          // 进程ID
	pid_t tgid;                         // 线程组ID
	char comm[S_COMMLEN];               // 进程命令名
	unsigned short size;                // 包的总大小：头部 + 命令参数信息
	struct ebpf_parent_info pinfo;      // 父进程信息
	unsigned long exeino;               // 可执行文件的inode号
	unsigned long proctime;             // 进程创建时间，作为进程标识
	struct timeval event_tv;            // 事件时间，例如连接时刻
	struct net_flags flags;             // 网络标志
	unsigned short sport;               // 源端口
	unsigned short dport;               // 目标端口
	struct sniper_ip srcip;             // 源IP地址
	struct sniper_ip dstip;             // 目标IP地址
	struct sniper_ipv6 srcipv6;         // 源IPv6地址
	struct sniper_ipv6 dstipv6;         // 目标IPv6地址
	unsigned int repeat;                // 重复次数
	int domain_query_type;              // 域名查询类型
	...
	unsigned short reason;              // 原因
	char ip[S_IPLEN];                   // IP地址
	char domain[S_DOMAIN_NAMELEN];      // 域名
};
