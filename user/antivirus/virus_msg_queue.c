...
void virus_msg_queue_destroy(void)
{
	...
	list_for_each_entry_safe(msg, m, &virus_msg_queue, list) {
		...
		sniper_free(msg, sizeof(struct virus_msg), SCAN_GET);
	}
...
int virus_msg_queue_full(void)
{
	...
	if (droped_full_msgs == 1) {
		MON_DBG2(DBGFLAG_ANTIVIRUS_QUEUE, "full queue(%d msgs), drop new virus msg\n",
			virus_msg_count);
		return 1;
	}
	...
	if (i >= report_dropfull_threshold) {
		MON_DBG2(DBGFLAG_ANTIVIRUS_QUEUE, "full queue(%d msgs), %d virus msgs droped\n",
			virus_msg_count, i);
...
static virus_msg_t *req2msg(virus_info_t *req)
{
	...
	msg = (virus_msg_t *)sniper_malloc(sizeof(struct virus_msg), SCAN_GET);
	if (msg == NULL) {
		MON_DBG2(DBGFLAG_ANTIVIRUS_QUEUE, "virus_msgd malloc msg fail\n");
		return NULL;
	}
	...
	if (msg->data == NULL) {
		MON_DBG2(DBGFLAG_ANTIVIRUS_QUEUE, "virus_msgd malloc databuf fail\n");
		sniper_free(msg, sizeof(struct virus_msg), SCAN_GET);
		return NULL;
	}
...
/* push msg to queue */
void virus_msg_queue_push(virus_info_t *req)
{
	virus_msg_t *msg = NULL;
