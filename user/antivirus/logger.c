...
//	printf("%s rotated\n", logfile);
	...
	chmod(logfile, 0666); //允许普通用户看，便于查错
}
...
void dbg_log(FILE *fp, const char *caption, const char *text)
{
	struct tm now = {0};
	time_t t = time(NULL);
	...
	fprintf(fp, "%04d%02d%02d%02d%02d%02d %d %s -> %s",
			now.tm_year+1900, now.tm_mon+1, now.tm_mday,
			now.tm_hour, now.tm_min, now.tm_sec, getpid(), caption, text);
...
void MON_DBG(const char *format, ...)
{
	va_list args;
	char text[LOG_LINE_MAX] = {0};
	...
	dbg_log(logfp, "INFO", text);
}

void MON_DBG2(char *flagfile, const char *format, ...)
{
	va_list args;
	char text[LOG_LINE_MAX] = {0};

	if (!logfp || !flagfile || access(flagfile, F_OK) != 0) {
		return;
	}
...
/* 日志文件没有记录任何日志时，删除该日志文件*/
void check_clean_log(void)
{
	if(!logfp || logname[0] == 0) {
		return;
	}
...
void print_tips(void)
{
	...
	printf("For more details, please see file:%s\n", logname);
	return;
}
