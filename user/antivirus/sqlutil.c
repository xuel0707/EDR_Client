...
sqlite3* connectDb(char *dbname, const char *crt_tbl_sql, char *pwd, int *first_time)
{
	...
	if (dbexist) {
		...
		if (rc == SQLITE_OK) {
			...
			if (rc == SQLITE_OK) {
				*first_time = 0;
				MON_DBG2(DBGFLAG_ANTIVIRUS, "open db %s ok\n", dbname);
				return db;
			}
		}
		if (db) {
		...
		MON_DBG2(DBGFLAG_ANTIVIRUS "open db %s fail: %s. rebuild it\n", dbname, sqlite3_errstr(rc));
		unlink(dbname);
	}
	...
	if (rc == SQLITE_OK) {
		...
		if (rc == SQLITE_OK) {
			*first_time = 1;
			MON_DBG2(DBGFLAG_ANTIVIRUS, "create db %s success\n", dbname);
			return db;
		}
		MON_DBG2(DBGFLAG_ANTIVIRUS, "create table in db %s fail: %s.   %s\n", dbname, sqlite3_errstr(rc), strerror(errno));
	}

	MON_DBG2(DBGFLAG_ANTIVIRUS, "create db %s fail: %s.   %s\n", dbname, sqlite3_errstr(rc), strerror(errno));
	if (db) {
		sqlite3_close_v2(db);
	}
	return NULL;
