...
void hydra_report_found_host(int32_t port, char *ip, char *svc, FILE *fp) {
  /*  char *keyw = "password";
    ...
    strcpy(ipaddr_str, hydra_address2string(ip));
    if (strcmp(svc, "smtp-enum") == 0)
      keyw = "domain";
    if (strcmp(svc, "rsh") == 0 || strcmp(svc, "oracle-sid") == 0)
      if (colored_output)
    ...
    hydra_get_next_login()); else if (strcmp(svc, "snmp3") == 0) if (colored_output)
        fprintf(fp, "[\e[31m%d\e[0m][\e[31m%s\e[0m] host: \e[32m%s\e[0m   login:
    \e[32m%s\e[0m\n", port, svc, ipaddr_str, hydra_get_next_password()); else
        fprintf(fp, "[%d][%s] host: %s   login: %s\n", port, svc, ipaddr_str,
    hydra_get_next_password()); else if (strcmp(svc, "cisco-enable") == 0 ||
    strcmp(svc, "cisco") == 0) if (colored_output) fprintf(fp,
    ...
    (stdout != fp) { if (strcmp(svc, "rsh") == 0 || strcmp(svc, "oracle-sid") == 0)
        printf("[%d][%s] host: %s   login: %s\n", port, svc, ipaddr_str,
    hydra_get_next_login()); else if (strcmp(svc, "snmp3") == 0) printf("[%d][%s]
    host: %s   login: %s\n", port, svc, ipaddr_str, hydra_get_next_password());
      else if (strcmp(svc, "cisco-enable") == 0 || strcmp(svc, "cisco") == 0)
